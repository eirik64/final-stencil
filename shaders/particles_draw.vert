...
const vec4 TRI_VERTS[NUM_VERTICES_PER_PARTICLE] = vec4[NUM_VERTICES_PER_PARTICLE](
    ...
    // vec4(p2.x, p2.y, 0, 0),
    // vec4(p3.x, p3.y, 0, 0)
     vec4(1, 2, 0, 0), // original p1: vec4(0, 2, 0, 0)
     vec4(0, 0, 0, 0), // original p2: vec4(0, 0, 0, 0)
     vec4(2, 1, 0, 0) // original p3: vec4(2, 0, 0, 0)
);
...
void main() {
    ...
    // get particleID and triID
    particleID = int(floor(gl_VertexID/3)); // cast int
    triID = int(mod(gl_VertexID, 3)); // mod, cast to int
    // Final Project: modify # of particles, positions are how they're distanced, offsets for individual models
...
//    texelFetch(, ivec)
    ...
    posTime += vec4(0.2, 0.7, 0, 0); // move up the particles' origin (originally this wasn't here for particles lab)
    ...
    gl_Position = anchorPoint + triPos - diameter * vec4(1.0, 1.0, 0.0, 0.0); // original last multiplier: vec4(0.5, 0.5, 0.0, 0.0)
}
